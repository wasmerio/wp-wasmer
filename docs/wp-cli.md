...
The optional token is used for an existing Wasmer account. Without it, the command creates a temporary app. Unless `--app-name` is supplied, the command generates a name in the form `wordpress-xxxxxxxx`, using eight random lowercase letters and numbers. The command waits for the app build, transfer, and target import to finish. On success, standard output is one standalone JSON document containing the target app information (`id`, `name`, `url`, `adminUrl`, `willPerishAt`, and `activeVersion.id`); failures are reported as WP-CLI errors.
