...
Connects WordPress installations hosted on Wasmer with the Wasmer platform.

== Description ==

Wasmer integrates a WordPress installation with Wasmer-managed hosting. On a managed Wasmer app it provides hosting status, cache controls, secure dashboard login, WP-CLI integration, and the destination side of Wasmer site migrations.
...
* The migration endpoints receive a database and site files from the separately installed Wasmer Migrate plugin. Transfer tokens are generated per migration session, and transferred staging data is removed after completion, cancellation, expiry, or uninstall.
